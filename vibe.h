...
#define COLOR_FOREGROUND 255 /*!< Default label for foreground pixels. Note that some authors chose any value different from 0 instead */
    ...
    int32_t libvibeModel_Sequential_Update_8u_C1R(
                                                  ...
                                                  uint8_t *BianJieJianCe,
                                                  uint8_t *grayMap_count
                                                  );
     ...
     * @return
     */
    typedef struct maskProProc
    {
        cv::Mat  segmentationMap; // in out
        cv::Mat  grayMapCount; //in out
        cv::Mat  sobelRes; // out
    }MASK_PRO_PROC;
    
    typedef struct maskProConfig
    {
        int grayAlpha;
        int keepingTime;
        bool isNeedRebuildForeground;
    }MASK_PROC_CONFIG;
    
    int32_t libvibeModel_Sequential_Update_8u_C1R_EX(
                                                     vibeModel_Sequential_t *model,
                                                     const cv::Mat& image_data,
                                                     cv::Mat& updating_mask,
                                                     cv::Mat& grayMap_count,
                                                     cv::Mat& grayMap,
                                                     MASK_PROC_CONFIG config
                                                     );
